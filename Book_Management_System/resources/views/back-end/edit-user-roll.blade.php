<div class="card card-primary mt-3">
    ...
    <form action="/update-user-roll/{{ $user_roll->id }}" id="update-user-roll" method="post">
        <div class="card-body">
            @csrf
            <div class="row">
                <div class="col-md-6">
                                ...
                                <td class="cheack-all">
                                    @foreach ($item['data'] as $data)
                                        <input class="form-check-input" type="checkbox" name="permission[]"
                                            value="{{ $data['permission'] }}" @if ($data['permission'] == 'dashboard') checked disabled @endif <?php if (in_array($data['permission'], json_decode($user_roll_permission))) {
                                                echo 'checked';
                                            } ?>>
                                        <label>
                                            {{ $data['title'] }}
